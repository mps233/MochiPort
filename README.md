...
第一次使用时，点击“扫码使用新机器人”，按二维码流程完成飞书接入。

接入成功后，飞书状态会显示为已连接。之后正常使用不需要反复扫码；断开后可以重新接入，只有更换机器人时才需要重新扫码。
...
不需要在 Codex App 的“连接”设置页里看到远程连接设备列表。这个项目走的是本地 backend + 飞书 bridge，只要 `Codex Remote` 的状态概览都正常，就可以直接在飞书里使用。
