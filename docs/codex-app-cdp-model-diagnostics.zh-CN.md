...
增强脚本 v8 在 React root render 之前监听 `window.__STATSIG__.firstInstance`。Statsig client 构造器
...
8. 启动接口以 Statsig 模型配置和关键 gate 已生效作为完成条件，最多等待 45 秒；`app routes mounted` 和 bootstrap 是否被拦截仅作为诊断信息，兼容 pre-login 路径。
