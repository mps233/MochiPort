...
- **加模型**：在 **AI 网关 → 概览 → Codex 可用模型** 里勾选内置模型，或点「新建自定义模型…」声明目录之外的模型（名称、服务商、上下文、是否支持图片）；服务商 `models` 列表里出现过的模型名也会自动列出来（标为「自动」）。新建服务商时若模型列表为空，保存前会自动向上游拉一次模型清单，并把上游声明的展示名、上下文窗口、图片能力一起记下来。加完用 **AI 网关** 的「启动 Codex」重启一次 Codex 即可看到，不需要改配置源码。
...
| 标准回复（默认） | 一个任务一条聚合气泡，原地更新：过程文案直接可见，工具步骤收进折叠的 `工具摘要（N）`，子代理、文件修改各自折叠 |
