...
- 一个 turn 只发一条聚合进度气泡：过程文案（Codex 的说明文字）在气泡里直接可见，工具/命令步骤收进折叠的「工具摘要（N）」块，原位置更新；完成或中断时收口，展开后过长历史只保留关键步骤和最近步骤。
- 同一 turn 的子代理活动并入同一条聚合气泡的「协作」折叠块，不再单独发一条消息；只显示名称、状态、耗时和简短结果，`wait` 轮询、内部 thread/call id、空数组及原始 JSON 不发送到聊天。
- 上下文压缩不再切分过程文案气泡：压缩提示仍单独一条，过程文案继续写进当前聚合气泡。
...
- 切块可能落在代码围栏中间：跨段时自动补齐闭合/起始围栏（并保留语言标注），避免后续分段按普通文本渲染。
- ``` 围栏的语言标注渲染为 `<code class="language-…">`，客户端可识别语言；非法字符被清洗。
- 续段标记跟随配置语言（中文"（未完待续）/（接上文）"，英文 "(continues...)/(continued)"）。
- 静默通知："完整"颗粒度的过程文本（`ImOutboundKind::Item/ImageItem`）与多段回复的续段携带 `disable_notification`，只有首段、最终回复和审批会响铃。
- 超长最终回复（切块数 > 2）改发 `.md` 文件附件，caption 为提示语和署名；附件发送失败自动回退为分段发送。
- 修复：`post_media_file` 此前写死官方 API 地址，自建 Bot API server 与测试钩子对媒体上传不生效，现统一走 `api_base`。

### 极简卡片排版（build 506）

选定“极简卡片”消息风格：装饰最少、留白多、层级靠字重和分隔线。

- 过程进度消息：顶部为状态卡头（🔄 任务进行中 / ❌ 失败态），下一行是卡片分隔线 `──────────────`，正文各 section 保持不变。
- 最终回复：顶部为 `✅ 已完成 · 耗时` 卡头（耗时取 `runtime.turn_started_at/turn_finished_at`，不足 1 秒或未知时只显示状态），加粗 + 分隔线 + 正文；正文后不再拼接署名行（`footer` 仅保留在文件附件的 caption 里）。
- 超长回复改发文件时，caption 仍为提示语 + 署名。
- 不改动 Codex 正文内容本身；草稿流式、切块续段、审批等机制不变。

### 单气泡过程卡（build 535）

标准颗粒度下，一个 turn 不再有“过程文案气泡 + 工具/子代理气泡”两条聚合消息，而是合并为一条：

- 顶部仍是状态卡头（`🔄 执行中 · N 步` / `❌ 执行失败 · N 步` / `执行完成 · N 步`）+ 分隔线 `──────────────`。
- 过程文案（commentary，即 Codex 的说明文字）直接以可见段落显示，超字符预算时从最早的条目开始丢弃并标注 `… 另外 N 条较早进展已省略`；`\`\`\`` 围栏按语言渲染为代码块。
- 工具/命令步骤统一收进一个默认折叠的 `工具摘要（N）` 块（`rich_blocks::details`，`is_open = false`）：内部保留“运行中/失败优先”的步骤挑选与 `… 另外 N 个较早步骤` 省略计数，失败输出仍是嵌套的折叠块。
- 计划、搜索、协作子代理、文件修改保持原表现：协作与文件修改本来就是折叠块，计划仍然可见。
- 思考摘要不再单独占标题行：它折成**一行**（超出截断并以 `…` 结尾）直接显示在卡头下方，并用等宽蓝底 `code` 呈现，与底部 `turn <id>` 同一种样式。
- 富消息 blocks 内不放装饰性 emoji（`✅ / ❌ / ⚠️ / ⏳ / 🛠 / 🔄` 只出现在 Markdown 回退文本与卡头），因此折叠标题是纯文字 `工具摘要（N）`。
- 富消息不可用时，回退文本按“过程文案 → 工具摘要 → 协作”顺序直出，无法折叠属预期差异。
- 过程文案与工具步骤按**到达序号交错**渲染（`思考过程（N）` 与 `工具摘要（N）` 交替出现），不会因为步骤变多而各自并成一大批。
- 最终回复**内嵌**在同一条气泡里，做成默认展开的「最终回复」折叠块（`rich_blocks::details`，`is_open = true`），不再单独发 `✅ 已完成` 气泡；turn 耗时拼到卡头（如 `执行完成 · 2 步 · 25秒`）。
- 最终回复内的 markdown（表格、标题、列表、链接、斜体、围栏代码）由 `commentary_entry_blocks` 转成对应的 blocks；非 http(s) 链接（如 `/Users/...` 本地路径）只保留链接文字，避免露出 `[文字](路径)` 原文。
