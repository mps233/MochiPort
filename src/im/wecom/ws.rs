                ...
                match frame? {
                    Message::Text(text) => {
                        ...
                        if is_event_frame(&value) {
                            if let Some(message) = normalize_card_event(
                                &api.settings,
                                &account_id,
                                &value,
                            ) {
...
async fn normalize_message(
    ...
    let raw_chat_id = match chat_type {
        ChatType::Direct => sender_id,
        ChatType::Group => body.get("chatid")?.as_str()?.trim(),
    };
    if !allowed(&api.settings.allowed_user_ids, sender_id)
        || !allowed(&api.settings.allowed_chat_ids, raw_chat_id)
    {
        return None;
    }
    let chat_id = wecom_chat_target(&chat_type, raw_chat_id);
    let attachments = match collect_attachments(state, msgtype, body).await {
        Ok(attachments) => attachments,
        Err(err) => {
            state
                .push_event(
                    "warn",
                    "wecom_attachment_rejected",
                    format!("account={account_id} chat={raw_chat_id} err={err}"),
                )
                .await;
            return None;
        }
...
fn normalize_card_event(
    ...
    let sender_id = body.pointer("/from/userid")?.as_str()?.trim().to_string();
    ...
    if !allowed(&settings.allowed_user_ids, &sender_id)
        || !allowed(&settings.allowed_chat_ids, &raw_chat_id)
    {
        return None;
    }
    let chat_id = wecom_chat_target(&chat_type, &raw_chat_id);
...
async fn collect_attachments(
    ...
    let bytes = tokio::time::timeout(ATTACHMENT_TIMEOUT, async {
        let response = crate::outbound_http::get()
            ...
            .error_for_status()
            .context("WeCom attachment returned an HTTP error")?;
        if response
            .content_length()
            .is_some_and(|length| length > MAX_ATTACHMENT_BYTES as u64)
        {
            anyhow::bail!("WeCom attachment exceeds the 50 MB limit");
        }
        ...
        while let Some(chunk) = stream
            ...
            .await
            .transpose()
            .context("failed to read WeCom attachment")?
        {
    ...
    .await
    .context("WeCom attachment download timed out")??;
    let bytes = match media.get("aeskey").and_then(Value::as_str) {
        Some(key) => decrypt_media(&bytes, key)?,
        None => bytes,
    };
    ...
    if tokio::fs::create_dir_all(&root).await.is_err() {
        anyhow::bail!("failed to create WeCom attachment directory");
    }
    let path = root.join(format!("{}_{}", now_ms(), sanitize_filename(name)));
    if tokio::fs::write(&path, bytes).await.is_err() {
        anyhow::bail!("failed to persist WeCom attachment");
    }
    Ok(vec![InboundAttachment {
...
fn decrypt_media(encrypted: &[u8], aes_key: &str) -> Result<Vec<u8>> {
    ...
    anyhow::ensure!(
        padding > 0 && padding <= 16 && padding <= decrypted.len(),
...
fn wecom_chat_target(chat_type: &ChatType, chat_id: &str) -> String {
    let prefix = match chat_type {
        ChatType::Direct => "single",
        ChatType::Group => "group",
    };
    format!("{prefix}:{}", chat_id.trim())
}
...
mod tests {
    ...
    async fn normalizes_direct_and_group_text_callbacks() {
        ...
        assert_eq!(direct.chat_id, "single:u1");
        ...
        let group = normalize_message(&state(), &api(), "a1", &group)
            .await
            .unwrap();
        assert_eq!(group.chat_id, "group:g1");
    ...
    fn normalizes_approval_card_event() {
        ...
        let inbound = normalize_card_event(&api().settings, "a1", &event).expect("card event");
        assert_eq!(inbound.chat_id, "single:u3");
        assert!(matches!(
            inbound.action,
    ...
    fn normalizes_nested_template_card_event() {
        ...
        let inbound =
            normalize_card_event(&api().settings, "a1", &event).expect("nested card event");
        assert!(matches!(
            inbound.action,
    ...
    fn normalizes_history_selection_card_event() {
        ...
        let inbound =
            normalize_card_event(&api().settings, "a1", &event).expect("selection card event");
        assert!(matches!(
            inbound.action,
    ...
    fn normalizes_thread_create_form_card_event() {
        ...
        let inbound =
            normalize_card_event(&api().settings, "a1", &event).expect("create form event");
        assert!(matches!(
            inbound.action,
    ...
    fn normalizes_thread_create_custom_cwd_card_event() {
        ...
        let inbound =
            normalize_card_event(&api().settings, "a1", &event).expect("custom cwd event");
        assert!(matches!(
            inbound.action,
            Some(InboundAction::ThreadRouteCreateSubmit {
                request_id,
    ...
    fn normalizes_enter_chat_event() {
        ...
        let inbound =
            normalize_card_event(&api().settings, "a1", &event).expect("enter chat event");
        assert_eq!(inbound.chat_id, "single:noreply");
        assert!(matches!(
            inbound.action,
    ...
    #[test]
    fn rejects_unauthorized_card_events() {
        let mut settings = api().settings;
        settings.allowed_user_ids = vec!["allowed-user".to_string()];
        let event = json!({
            "cmd": "aibot_event_callback",
            "headers": { "req_id": "event-denied" },
            "body": {
                "chattype": "single",
        ...
        assert!(normalize_card_event(&settings, "a1", &event).is_none());
    }

    #[test]
    fn decrypts_url_safe_unpadded_media_key() {
        use aes::cipher::{BlockEncrypt, KeyInit};
        use base64::Engine as _;

        let key = [7_u8; 32];
        let plaintext = b"wecom media";
        let padding = 16 - (plaintext.len() % 16);
        let mut padded = plaintext.to_vec();
        padded.extend(std::iter::repeat_n(padding as u8, padding));
        let cipher = aes::Aes256::new_from_slice(&key).expect("AES key");
        let mut previous = key[..16].to_vec();
        ...
        assert_eq!(decrypt_media(&padded, &encoded).unwrap(), plaintext);
    }
}
