...
// Modules stay crate-visible: the only supported entry point for external
// callers is `run()`. Promoting a module to `pub` widens the public API and
// requires its internal types to be public too, so it should be a deliberate
// decision rather than a default.
pub(crate) mod ai_gateway;
pub(crate) mod app_state;
pub(crate) mod bridge;
pub(crate) mod chain_log;
pub(crate) mod cli;
pub(crate) mod codex;
...
pub(crate) mod remote_control_backend;
pub(crate) mod store;
...
/// Runs the MochiPort CLI: the daemon, the bridge on/off controls, and the
/// Codex desktop integration commands. The `mochiport` binary is a thin wrapper
/// over this entry point so the implementation is reusable and testable as a
/// library instead of only as a binary.
pub fn run() -> anyhow::Result<()> {
    ...
    if let Command::MigrateStorage { source } = &cli.command {
        ...
        let report = daemon::storage_migration::migrate_storage(source.clone())?;
        if report.migrated {
            println!(
...
async fn async_main(cli: Cli) -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::InspectVsCodeRemoteControl => {
            print_vscode_remote_control_report(codex::vscode_patch::inspect_remote_control()?);
            Ok(())
        }
        Command::EnableVsCodeLegacyPatch => {
            print_vscode_remote_control_report(codex::vscode_patch::enable_legacy_patch_fallback()?);
            Ok(())
        }
        Command::RestoreVsCodeLegacyPatch => {
            print_vscode_remote_control_report(
                codex::vscode_patch::restore_legacy_patch_fallback()?
            );
            Ok(())
        }
        Command::ConfigureCodexApp {
            ...
            let report = codex::app_config::configure_codex_app(
        ...
        Command::UninstallCodexApp { codex_home } => {
            let backend_url = config.remote_control_base_url();
            let report = codex::app_config::uninstall_codex_app(codex_home, &backend_url)?;
...
async fn run_daemon(config_path: PathBuf, config: AppConfig) -> anyhow::Result<()> {
    let daemon_identity = daemon::process::DaemonIdentity::new();
    let _daemon_lock =
        daemon::process::DaemonInstanceLock::acquire(&config_path, &daemon_identity)?;
    ...
    if desktop_integration_enabled {
        ...
        let result = tokio::task::spawn_blocking(move || {
            codex::app_config::inspect_gui_api_base_url(&backend_url)
        })
        .await;
    ...
    let _active_daemon_locator = web::manage::publish_active_daemon_locator(
        &state.config_path,
...
fn config_path_from_cli(path: Option<PathBuf>) -> PathBuf {
    path.map(absolutize)
        .unwrap_or_else(daemon::storage_migration::current_config_path)
}
...
async fn notify_daemon_bridge(
    ...
    let token = web::manage::management_token(config_path)
...
fn print_vscode_remote_control_report(report: codex::vscode_patch::VsCodeExtensionPatchReport) {
