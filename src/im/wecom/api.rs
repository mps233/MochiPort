...
impl WecomApi {
    ...
    pub async fn send_markdown(&self, chat_id: &str, content: &str) -> Result<String> {
        let (chat_type, chat_id) = parse_chat_target(chat_id)?;
        let response = self
            .request(
                "aibot_send_msg",
                None,
...
fn parse_chat_target(target: &str) -> Result<(u8, &str)> {
    let (kind, chat_id) = target
        .trim()
        .split_once(':')
        .context("WeCom chat target must be prefixed with single: or group:")?;
    if chat_id.is_empty() {
        bail!("WeCom chat target has an empty chat id");
    }
    match kind {
        "single" => Ok((1, chat_id)),
        "group" => Ok((2, chat_id)),
        _ => bail!("unsupported WeCom chat target kind: {kind}"),
    }
}
...
mod tests {
    ...
    #[tokio::test]
    async fn proactive_markdown_includes_direct_chat_type() {
        let command =
            capture_command(|api| async move { api.send_markdown("single:user-1", "hello").await })
                .await;
        assert_eq!(command.0, "aibot_send_msg");
        assert_eq!(command.2["chatid"], "user-1");
        assert_eq!(command.2["chat_type"], 1);
    }

    #[tokio::test]
    async fn proactive_markdown_includes_group_chat_type() {
        let command =
            capture_command(|api| async move { api.send_markdown("group:room-1", "hello").await })
                .await;
        assert_eq!(command.2["chatid"], "room-1");
        assert_eq!(command.2["chat_type"], 2);
    }

    #[test]
    fn rejects_untyped_chat_targets() {
        assert!(parse_chat_target("user-1").is_err());
        assert_eq!(parse_chat_target("group:room-1").unwrap(), (2, "room-1"));
    }
}
