...
async function runOfflineStatsigReload(session, useLocalPayload) {
  ...
  if (useLocalPayload) {
    ...
    if (!response.ok) throw new Error(`MochiPort Statsig bootstrap returned HTTP ${response.status}`);
    const envelope = await response.json();
    if (typeof envelope.statsigPayload !== "string") throw new Error("MochiPort Statsig bootstrap payload is missing");
