...
OpenAI 的 `prompt_cache_key` 不能直接映射成 Anthropic cache。

Anthropic prompt caching 使用 `cache_control` 标记 cacheable block。第一阶段策略：

- 不自动把所有请求加 cache_control。
- 对稳定 system / large tool definitions 后续可配置启用 `cache_control: {"type":"ephemeral"}`。
...
- prompt caching 的 cache_control 自动策略需要单独配置，不能用 OpenAI `prompt_cache_key` 直接推导。
