...
- [ ] 迁移 Codex App 接入、修复和卸载操作。
- [ ] 迁移增强启动及其预检、等待、取消和失败恢复。
- [ ] 迁移可见模型配置。
- [ ] 迁移会话历史、搜索、状态和 Provider 移动操作。
...
- [ ] 迁移 Provider 列表、启停、添加、编辑、删除、协议和权重。
- [ ] 迁移图像生成过滤、请求日志和详情日志开关。
- [ ] 迁移请求日志分页表格、筛选、搜索和清理。
- [ ] 迁移请求详情的 Codex 请求、上游请求、SSE、响应和错误视图。
...
- [ ] 使用 SwiftUI `Settings` 迁移语言、外观、本地连接和出站代理设置。
- [ ] 迁移诊断导出、打开日志和版本信息。
- [ ] 先交付手动检查更新：发现新版本、展示发布说明并打开签名下载页，不在 App 内下载或替换程序。
