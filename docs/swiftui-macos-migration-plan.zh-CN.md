...
| 项目 | 内容 |
| --- | --- |
| 状态 | Phase 3 已完成；Phase 2 的内嵌 daemon、生命周期租约、安全重启、菜单栏、关闭偏好和 GUI supervisor 已落地；Phase 4-6 的核心页面已接入真实版本化管理 API，请求日志服务端分页、诊断导出和发布级无障碍收尾仍待完成 |
| 最近核验 | 2026-08-15；SwiftPM 100 项通过；Universal Release build 402 已完成 GUI 自动恢复、更新切换防竞态、daemon/GUI 构建号显示、会话项目分组、AI 网关改版和请求日志同页详情的正式组装、签名与运行验证 |
| 目标版本 | 0.5.x 预览阶段 |
| 主平台 | macOS 13 及以上 |
| 主前端 | SwiftUI |
| 核心后端 | 现有 Rust daemon |
...
当前状态：Provider 和网关开关已可真实读写，API Key 只写不回显；请求日志支持最近记录、客户端搜索、同页按需脱敏详情、复制、详情查找、惰性逐行渲染、不可逆清空确认和删除数量反馈。服务端游标分页、组合筛选和排序尚未完成，因此日志列表复合条目保持未勾选。
