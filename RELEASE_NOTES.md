...
- UI 与 daemon 使用独立版本和构建号，并分别检查可用更新。
- 设置中提供统一更新入口；daemon 更新会在用户确认后等待受保护任务完成，再安全切换。
- 发布流程会生成独立的 UI 与 daemon 更新元数据。

## 界面与数据

- 概览精简无用状态区块，本地服务信息迁移至更新设置。
- Sub2API 主账号汇总保持可见，子账号列表可独立展开和收起。
- 修复最近七天用量趋势、当前中转站余额与倍率，以及跨页面背景不一致。
- 使用 macOS 26 原生滚动边缘效果，让标题栏材质随内容和环境色变化。
- 增加自适应明暗外观的 MochiPort 图标，并修复 GUI 恢复流程可能重复启动窗口的问题。

---

## MochiPort v0.5.2（历史发行记录）
